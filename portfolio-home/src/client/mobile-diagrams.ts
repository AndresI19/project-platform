...
//
// WHY. The desktop diagrams are pinned to a wide grid (see architecture.ts); scaling one to a phone
// gives a shape you zoom into, not read, so a phone gets a transit map drawn for it. The desktop
// diagram is already topological, so the map was a transit map before anyone drew rails on it.
//
// WHAT MAPS TO WHAT. Services are stations; routes are lines; nginx is an interchange; the VM and the
// cluster are fare zones; volumes are depot sidings (dashed, terminating); sealed-secrets is the third
// rail, touching everything and carrying nobody.
//
// THE DEBT. This is a SECOND hand-maintained picture of the same platform — exactly what rotted the
// old '@media (max-width:900px)' block, where adding a service meant editing both or colliding two
// boxes. So: A CHANGE TO A DESKTOP DIAGRAM IS NOT DONE UNTIL THE MOBILE ONE MATCHES. The vmcp
// interchange is the proof — the desktop draws only the agent arrow into vmcp while the box advertises
// /vmcp/ as a link, though nginx routes BOTH '/vmcp/' (browser) and '/mcp' (agent) to it.
//
// Every class/property is prefixed per diagram (mt-/mc-/ma-) and scoped under .arch-mobile: the three
// maps reuse names like "trunk" and "stn" for different strokes, so one vocabulary would fight.
...
<svg viewBox="0 0 366 620" role="img" aria-label="The platform as a transit line diagram. A browser route and an agent route share a trunk from the internet, through Cloudflare and an outbound tunnel, into an nginx interchange. nginx fans out to four peer services on one rank: home, quiz, platform-auth and vmcp. The browser route serves all four and terminates. The agent route serves vmcp alone, so vmcp is an interchange where both lines stop, and the agent route continues past it to rs-mcp-server and on to the public APIs. Three volumes hang below the rank as sidings, and a sealed-secrets third rail feeds every station without carrying traffic.">
    <!-- ZONES: full-width bands with an inset label (no hand-sized pill). Labels start RIGHT OF THE
         TRUNK — the trunk descends at x=96–108, so a label from the band's left edge is read through by
         two lines. The bands end above the terminus so the arrow to the public APIs genuinely LEAVES
         the cluster, not stops on its border — the tunnel story's whole claim is the boundary is real. -->
    ...
    <!-- THE FAN. Four peers, ONE rank (the desktop's row 11). The red line reaches vmcp too: nginx
         routes '/vmcp/' to its dashboard (browser) and '/mcp' to its gateway (agent), so BOTH lines
         stop there. Red runs its distributor ABOVE blue's at y=340, right of the blue trunk, so it
         reaches x=312 crossing nothing; every leg is orthogonal (down, across, down). -->
    ...
    <!-- THIRD RAIL. The rank is at y=416, so the feeder sits above it at 396 and stubs down to each
         station (at y=440 the taps ran straight through the label band). -->
    ...
    <!-- THE RANK. Labels HORIZONTAL and centred under each station, name over path (rotating them to
         make room only hides that there is no room). A STATION CARRIES A NAME, NOT AN ADDRESS: four
         names plus four paths can't share a 366px rank ("/cloud-developer-quiz/" alone is 82px), so the
         paths are drawn as one routing table in the key below — as the form works anyway, the strip
         above a train door names stops, fares and zones live on a panel beside it. -->
    <g text-anchor="middle">
      ...
      <!-- THE INTERCHANGE. Two lines stop here, so it's a capsule with one dot per line, not a
           single-line circle. Blue TERMINATES (the dashboard), red CONTINUES to rs-mcp. -->
    ...
    <!-- SIDINGS: below the rank, never through it. Each label sits BELOW its siding (a beside-label ran
         through the fork's own text). The fork visibly serves two stations, so it is its own label. -->
    ...
    <!-- DOWNSTREAM OF vmcp — the only thing the red line reaches ALONE; blue stopped at the interchange.
         Red EXITS SIDEWAYS and descends the far-right lane at x=344, clear of the label band (dropping
         straight from the capsule at x=312 runs through "vmcp"), which is also the truer subway idiom. -->
...
<svg viewBox="0 0 366 700" role="img" aria-label="The deploy pipeline as a transit line diagram, running top to bottom through three zones. In GitHub's zone, a service repo is an interchange where two routes begin: an amber pull-request route and a crimson merge route. Amber stops at the CI job, crimson at version-tag and release, and both meet again at the job queue, which dispatches by label. There the routes part: amber goes to a GitHub-hosted VM and ends at a buffer stop, because a pull request never deploys. Crimson crosses into the second zone, this machine outside the cluster, where a hatched single-track section admits one job at a time to the self-hosted runner. The runner pushes an image to the registry depot in the left-hand lane, reports to Discord, and calls helm upgrade, which crosses into the third zone, the cluster. There the apiserver makes a new ReplicaSet, the kubelet pulls the image back out of the depot along the dashed lane, and a Job watches the rollout. A separate dashed line shows the runner polling GitHub outbound, so GitHub never dials in.">
    ...
    <!-- THE THREE ZONES. Stacked and adjacent, not nested: three trust domains, none inside another
         (the topology map nests, because the cluster really sits inside the workstation). -->
    ...
    <!-- THE LANES. Amber and crimson run 12px apart down a left gutter; every label lives right of x=76
         — the one layout at 366px where four words and a line never argue. -->
    <path class="mc-trunk mc-t-pr"  d="M44 70 V 215"/>
    <path class="mc-trunk mc-t-rel" d="M56 70 V 215"/>

    <!-- ① Service repos — both routes begin here, so an interchange: a capsule with one dot per line. -->
    ...
    <!-- ③ THE JUNCTION. Below the queue the routes physically part — what "by label" MEANS. Amber ends
         at a buffer stop: a pull request never reaches a pod, the most important thing here. -->
    ...
    <!-- (1) THE RUNNER POLLS. Outbound, its own dashed lane, arrowhead UP into GitHub's zone (the same
         claim cloudflared makes on the topology map). GitHub is never given a route in. -->
    ...
    <!-- ④ SINGLE-TRACK SECTION — the signature. Literally single-track working: the section admits one
         train, the next waits at the boundary until it clears. The pinch is the whole argument. -->
    ...
    <!-- Crimson runs UNBROKEN between zone boundaries (a gap here stops the line through the very
         section this diagram is about). The hatch is painted OVER the line, not instead of it. -->
    ...
    <!-- ⑤ THE DEPOT. An image is stabled here and fetched later — the runner never hands it over. It
         lives in the LEFT gutter so both accesses (runner's push, kubelet's pull) enter no label's lane. -->
    ...
    <!-- ⑥ THE CLUSTER. Causal order: helm upgrade → apiserver makes a ReplicaSet and schedules a Pod →
         only THEN the kubelet pulls the image. The desktop stacks the kubelet above the apiserver,
         which reads as the kubelet scheduling; the order below is what the boxes' own words describe. -->
    <path class="mc-trunk mc-t-rel" d="M56 516 V 644"/>

    <!-- "Schedules a Pod for an image the node has yet to pull" belongs to the APISERVER — hanging it
         under the kubelet says the node schedules its own work. -->
...
<svg viewBox="0 0 366 620" role="img" aria-label="Auth drawn as a ticket and the barriers that read it. In the upper section, the ticket office: a passenger with no ticket in local storage goes to platform-auth, which verifies the password against platform-db and signs an RS256 token. A dotted branch shows continue-as-guest bypassing the office entirely and ending at a marker rather than a ticket. In the middle, the ticket itself is drawn, carrying a subject, an admin flag and an RS256 signature, kept under one local-storage key shared by all three front ends. In the lower section, three barriers stand side by side: home has no barrier because it has no gated routes, while quiz and vmcp each hold their own cached copy of the public keys, posted once from the office along a dashed line, and check the ticket themselves without calling the office.">
    ...
    <!-- One unbroken line from "no ticket" to the barriers — a break here leaves the thing the diagram
         is about sitting beside the line rather than on it. -->
    ...
    <!-- THE GUEST PATH bypasses the office: a guest marker is not a ticket, the office is never called.
         Dotted, right-hand lane, ending at a marker not a ticket. -->
    ...
    <!-- ② THE TICKET ITSELF. Everything above produces it, everything below reads it. A perforated
         stub, where the claims live. The perforation sits at x=250, dividing stub from body text. -->
    <g>
      ...
      <!-- The stub carries the validation mark. The localStorage facts live in the caption — they're
           about where the ticket LIVES, not what's printed on it. -->
    ...
    <!-- The keys are POSTED to the barriers once: one line, two barriers — the same fetch. It must
         leave the OFFICE, not the ticket's corner (that would say the ticket carries the key that
         validates it — the design inverted). It descends from the office zone's edge (two sides, not
         three, or it becomes a box drawn around the ticket) and STOPS at quiz — never reaching home's
         approach at x=82, because home holds no key and checks nothing. -->
